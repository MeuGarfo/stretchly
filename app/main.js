  ...
  if (!breakPlanner.isPaused) {
    ...
    if (settings.get('microbreak')) {
      submenu = submenu.concat([{
        label: i18next.t('main.microbreak'),
        click: function () {
    ...
    if (breakPlanner && breakPlanner.scheduler) {
      if (breakPlanner.isPaused) {
        ...
        if (breakType) {
          ...
          statusMessage += i18next.t('main.timeToNext', {'timeLeft': Utils.formatTillBreak(breakPlanner.scheduler.timeLeft + notificationTime), 'breakType': breakType })
          if (breakType === 'microbreak') {
            ...
            statusMessage +=  i18next.t('main.nextBreakFollowing', {'microbreakNumber': breakInterval - breakNumber})
          }
        }
      }
    }
